...
def employ(request):
    if request.method=="POST":
        ...
        return render(request,'empl.html',{'en':en})
def empd(request,id):
    if request.method=="POST":
    ...
    emd=Employe.objects.get(id=id)
    return render(request,"epd.html",{'ep':emd})
def eped(request,id):
...
def empr(request):
    ...
    return render(request,"empr.html",{'en':en})
def atr(request):
    return render(request,'atr.html')
def search(request):
     en=Employe.objects.filter()
     return render(request,'srch.html',{'en':en})